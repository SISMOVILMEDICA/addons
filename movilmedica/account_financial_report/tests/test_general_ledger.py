...
class TestGeneralLedgerReport(common.TransactionCase):
    ...
    def _get_report_lines(self, with_partners=False):
        ...
        res_data = self.env["report.account_financial_report.general_ledger"]._get_report_values(
            general_ledger, data
        )
        return res_data
    ...
    def test_01_account_balance(self):
        ...
        check_income_account = self.check_account_in_report(self.income_account.id, general_ledger)
        ...
        receivable_fin_balance = self._get_final_balance(self.receivable_account.id, general_ledger)
        ...
        receivable_fin_balance = self._get_final_balance(self.receivable_account.id, general_ledger)
        income_init_balance = self._get_initial_balance(self.income_account.id, general_ledger)
        income_fin_balance = self._get_final_balance(self.income_account.id, general_ledger)
    ...
    def test_03_unaffected_account_balance(self):
        ...
        unaffected_fin_balance = self._get_final_balance(self.unaffected_account.id, general_ledger)
    ...
    def test_validate_date_range(self):
        ...
        wizard = self.env["general.ledger.report.wizard"].create({"date_range_id": dr.id})
