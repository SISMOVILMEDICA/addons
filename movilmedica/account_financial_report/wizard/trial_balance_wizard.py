...
class TrialBalanceReportWizard(models.TransientModel):
    ...
    hide_parent_hierarchy_level = fields.Boolean("Do not display parent levels", default=False)
    ...
    def _check_show_hierarchy_level(self):
        for rec in self:
            if rec.hierarchy_on != "none" and rec.show_hierarchy_level <= 0:
                raise UserError(_("The hierarchy level to filter on must be " "greater than 0."))
    ...
    def onchange_company_id(self):
        ...
        if self.company_id and self.journal_ids:
            self.journal_ids = self.journal_ids.filtered(lambda a: a.company_id == self.company_id)
