...
class AgedPartnerBalanceReport(models.AbstractModel):
    ...
    @api.model
    def _calculate_amounts(self, ag_pb_data, acc_id, prt_id, residual, due_date, date_at_object):
    ...
    def _get_move_lines_data(
        ...
        if date_at_object < date.today():
            ...
            if acc_partial_rec:
                ...
                debit_ids = list(map(operator.itemgetter("debit_move_id"), acc_partial_rec))
                credit_ids = list(map(operator.itemgetter("credit_move_id"), acc_partial_rec))
    ...
    def _calculate_percent(self, aged_partner_data):
        for account in aged_partner_data:
            if abs(account["residual"]) > 0.01:
                ...
                account.update(
                    {
                        "percent_current": abs(round((account["current"] / total) * 100, 2)),
                        "percent_30_days": abs(round((account["30_days"] / total) * 100, 2)),
                        "percent_60_days": abs(round((account["60_days"] / total) * 100, 2)),
                        "percent_90_days": abs(round((account["90_days"] / total) * 100, 2)),
                        "percent_120_days": abs(round((account["120_days"] / total) * 100, 2)),
                        "percent_older": abs(round((account["older"] / total) * 100, 2)),
                    }
                )
            else:
                account.update(
