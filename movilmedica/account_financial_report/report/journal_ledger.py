...
class JournalLedgerReport(models.AbstractModel):
    ...
    def _get_move_lines_data(self, ml, wizard, ml_taxes):
        base_debit = base_credit = tax_debit = tax_credit = base_balance = tax_balance = 0.0
        if ml.tax_exigible:
    ...
    def _get_move_lines(self, move_ids, wizard, journal_ids):
        ...
        for ml in move_lines:
            ...
            taxes = (
                ml.id in move_line_ids_taxes_data.keys() and move_line_ids_taxes_data[ml.id] or {}
            )
            Move_Lines[ml.move_id.id].append(self._get_move_lines_data(ml, wizard, taxes))
            ...
            for report_move_line in report_move_lines:
                ...
                for tax in taxes:
                    ...
                    for field_key in field_keys:
                        journals_taxes_data[journal_id][tax.id][field_key] += ml_data[field_key]
    ...
    def _get_report_values(self, docids, data):
        ...
        journal_moves_data = {}
        for key, items in itertools.groupby(moves_data, operator.itemgetter("journal_id")):
            if key not in journal_moves_data.keys():
                journal_moves_data[key] = []
            journal_moves_data[key] += list(items)
        move_lines_data = (
            account_ids_data
        ) = partner_ids_data = currency_ids_data = tax_line_ids_data = move_line_ids_taxes_data = {}
        if move_ids:
