...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_html(self):
        ...
        rcontext["o"] = wiz
        result["html"] = self.env.ref("account_financial_report.report_trial_balance").render(
            rcontext
        )
        return result
    ...
    def _get_pl_initial_balance(
        ...
        for initial_balance in initial_balances:
            ...
            if foreign_currency:
                pl_initial_currency_balance += round(initial_balance["amount_currency"], 2)
    ...
    def _compute_account_amount(
        ...
        for tb in tb_period_acc:
            ...
            if foreign_currency:
                ...
                total_amount[acc_id]["ending_currency_balance"] = round(tb["amount_currency"], 2)
        ...
        for tb in tb_period_prt:
            ...
            if tb["partner_id"]:
                ...
                if tb["partner_id"] not in partners_ids:
                    partners_data.update({prt_id: {"id": prt_id, "name": tb["partner_id"][1]}})
    ...
    def _get_data(
        ...
        for account_rg in tb_initial_acc_rg:
            element = list(
                filter(
                    lambda acc_dict: acc_dict["account_id"] == account_rg["account_id"][0],
                    tb_initial_acc,
                )
            )
            if element:
        ...
        if foreign_currency:
            total_amount[unaffected_id]["ending_currency_balance"] += pl_initial_currency_balance
            total_amount[unaffected_id]["initial_currency_balance"] += pl_initial_currency_balance
        return total_amount, accounts_data, partners_data

    def _get_hierarchy_groups(self, group_ids, groups_data, old_groups_ids, foreign_currency):
        new_parents = False
        for group_id in group_ids:
            if groups_data[group_id]["parent_id"]:
                ...
                if nw_id in groups_data.keys():
                    ...
                    groups_data[nw_id]["ending_balance"] += groups_data[group_id]["ending_balance"]
                    if foreign_currency:
                        groups_data[nw_id]["initial_currency_balance"] += groups_data[group_id][
                            "initial_currency_balance"
                        ]
                        groups_data[nw_id]["ending_currency_balance"] += groups_data[group_id][
                            "ending_currency_balance"
                        ]
                else:
                    groups_data[nw_id] = {}
                    groups_data[nw_id]["initial_balance"] = groups_data[group_id]["initial_balance"]
                    groups_data[nw_id]["debit"] = groups_data[group_id]["debit"]
                    groups_data[nw_id]["credit"] = groups_data[group_id]["credit"]
                    groups_data[nw_id]["balance"] = groups_data[group_id]["balance"]
                    groups_data[nw_id]["ending_balance"] = groups_data[group_id]["ending_balance"]
    ...
    def _get_groups_data(self, accounts_data, total_amount, foreign_currency):
        ...
        for group_id in account_group_relation.keys():
            for account_id in account_group_relation[group_id]:
                ...
                if foreign_currency:
                    groups_data[group_id]["initial_currency_balance"] += total_amount[account_id][
                        "initial_currency_balance"
                    ]
                    groups_data[group_id]["ending_currency_balance"] += total_amount[account_id][
                        "ending_currency_balance"
                    ]
        ...
        for group in groups:
            ...
            for account in accounts_data.values():
                if group.code_prefix_start == account["code"][:len_group_code]:
                    ...
                    if foreign_currency:
                        groups_data[group_id]["initial_currency_balance"] += total_amount[acc_id][
                            "initial_currency_balance"
                        ]
                        groups_data[group_id]["ending_currency_balance"] += total_amount[acc_id][
                            "ending_currency_balance"
                        ]
        return groups_data
    ...
    def _get_report_values(self, docids, data):
        ...
        if not show_partner_details:
            ...
            if hierarchy_on == "relation":
                groups_data = self._get_groups_data(accounts_data, total_amount, foreign_currency)
            ...
            if foreign_currency:
                for account_id in accounts_data.keys():
                    ...
                    total_amount[account_id]["currency_name"] = accounts_data[account_id][
                        "currency_name"
                    ]
        return {
            "doc_ids": [wizard_id],
