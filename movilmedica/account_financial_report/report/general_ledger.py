...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_taxes_data(self, taxes_ids):
        ...
        for tax in taxes:
            ...
            taxes_data[tax.id]["tax_name"] = (
                tax.display_name + " (" + str(tax.amount) + taxes_data[tax.id]["string"] + ")"
            )
        return taxes_data
    ...
    def _get_initial_balance_data(
        ...
        gl_initial_acc = self._get_accounts_initial_balance(initial_domain_bs, initial_domain_pl)
        ...
        if gl_initial_acc_prt:
            for gl in gl_initial_acc_prt:
                ...
                gen_ld_data[acc_id][prt_id]["init_bal"]["bal_curr"] = gl["amount_currency"]
                gen_ld_data[acc_id][prt_id]["fin_bal"]["bal_curr"] = gl["amount_currency"]
        ...
        gen_ld_data[unaffected_id]["init_bal"]["balance"] += pl_initial_balance["balance"]
        ...
        gen_ld_data[unaffected_id]["fin_bal"]["balance"] += pl_initial_balance["balance"]
        if foreign_currency:
            gen_ld_data[unaffected_id]["init_bal"]["bal_curr"] += pl_initial_balance["bal_curr"]
            gen_ld_data[unaffected_id]["fin_bal"]["bal_curr"] += pl_initial_balance["bal_curr"]
    ...
    def _get_move_line_data(self, move_line):
        move_line_data = {
            ...
            "rec_name": move_line["full_reconcile_id"][1] if move_line["full_reconcile_id"] else "",
        ...
        if move_line_data["ref"] == move_line_data["name"] or move_line_data["ref"] == "":
    ...
    def _get_period_ml_data(
        ...
        move_lines = self.env["account.move.line"].search_read(domain=domain, fields=ml_fields)
        journal_ids = set()
        ...
        for move_line in move_lines:
            ...
            if acc_id not in gen_ld_data.keys():
                gen_ld_data = self._initialize_account(gen_ld_data, acc_id, foreign_currency)
            if acc_id in acc_prt_account_ids:
                ...
                gen_ld_data[acc_id][prt_id]["fin_bal"]["balance"] += move_line["balance"]
                if foreign_currency:
    ...
    @api.model
    def _recalculate_cumul_balance(self, move_lines, last_cumul_balance, rec_after_date_to_ids):
        for move_line in move_lines:
    ...
    def _create_general_ledger(
        ...
        for acc_id in gen_led_data.keys():
            ...
            if not gen_led_data[acc_id]["partners"]:
                account = self._create_account(account, acc_id, gen_led_data, rec_after_date_to_ids)
                if (
                    hide_account_at_0
                    and float_is_zero(
                        gen_led_data[acc_id]["init_bal"]["balance"],
                ...
                if show_partner_details:
                    ...
                    for prt_id in gen_led_data[acc_id].keys():
                        ...
                        if not isinstance(prt_id, int):
                            ...
                            for ml_id in gen_led_data[acc_id][prt_id].keys():
                                if not isinstance(ml_id, int):
                                    partner.update({ml_id: gen_led_data[acc_id][prt_id][ml_id]})
                                else:
                                    move_lines += [gen_led_data[acc_id][prt_id][ml_id]]
                            move_lines = sorted(move_lines, key=lambda k: (k["date"]))
                            move_lines = self._recalculate_cumul_balance(
    ...
    def _calculate_centralization(self, centralized_ml, move_line, date_to):
        ...
        centralized_ml[jnl_id][month]["balance"] += move_line["debit"] - move_line["credit"]
