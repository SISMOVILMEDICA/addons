...
class JournalLedgerXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        ...
        if report.with_account_name:
            columns.append({"header": _("Account Name"), "field": "account_name", "width": 15})
    ...
    def _get_report_filters(self, report):
        target_label_by_value = {
            value: label
            for value, label in self.env["journal.ledger.report.wizard"]._get_move_targets()
        }

        sort_option_label_by_value = {
            value: label
            for value, label in self.env["journal.ledger.report.wizard"]._get_sort_options()
        }
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.journal_ledger"]._get_report_values(
            report, data
        )
        group_option = report.group_option
        if group_option == "journal":
            for ledger in res_data["Journal_Ledgers"]:
                self._generate_journal_content(workbook, report, res_data, ledger, report_data)
    ...
    def _generate_journal_content(self, workbook, report, res_data, ledger, report_data):
        journal = self.env["account.journal"].browse(ledger["id"])
        currency_name = (
            journal.currency_id and journal.currency_id.name or journal.company_id.currency_id.name
        )
    ...
    def _generate_no_group_taxes_summary(self, workbook, report, res_data, report_data):
        self._generate_taxes_summary(workbook, "Tax Report", res_data["tax_line_data"], report_data)
    ...
    def _generate_journal_taxes_summary(self, workbook, ledger, report_data):
        ...
        self._generate_taxes_summary(workbook, sheet_name, ledger["tax_lines"], report_data)

    def _generate_moves_content(self, workbook, sheet_name, report, res_data, moves, report_data):
        ...
        for move in moves:
            for line in move["report_move_lines"]:
                ...
                line["partner"] = self._get_partner_name(line["partner_id"], partner_ids_data)
                ...
                line["taxes_description"] = report._get_ml_tax_description(
                    ...
                    res_data["move_line_ids_taxes_data"].get(line["move_line_id"], False),
                )
                self.write_line_from_dict(line, report_data)
            report_data["row_pos"] += 1

    def _generate_taxes_summary(self, workbook, sheet_name, tax_lines_dict, report_data):
        report_data["workbook"] = workbook
        report_data["sheet"] = workbook.add_worksheet(sheet_name)
