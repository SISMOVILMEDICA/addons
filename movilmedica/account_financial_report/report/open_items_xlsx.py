...
class OpenItemsXslx(models.AbstractModel):
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        res_data = self.env["report.account_financial_report.open_items"]._get_report_values(
            report, data
        )
        # For each account
            ...
            if Open_items[account_id]:
                if show_partner_details:
                    for partner_id in Open_items[account_id]:
                        type_object = "partner"
                        # Write partner title
                        self.write_array_title(partners_data[partner_id]["name"], report_data)
                        ...
                        partners_data[partner_id].update(
                            {
                                ...
                                "currency_name": accounts_data[account_id]["currency_name"],
                            }
                        )
