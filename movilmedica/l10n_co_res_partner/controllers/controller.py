...
class controller(http.Controller):
    @http.route(
        "/l10n_co_res_partner/get_partner_state_city/",
        methods=["POST"],
        type="json",
        auth="public",
        website=True,
    ...
    @http.route(
        "/l10n_co_res_partner/get_partner_state/",
        methods=["POST"],
        type="json",
        auth="public",
        website=True,
    ...
    @http.route(
        "/l10n_co_res_partner/get_state_city/",
        methods=["POST"],
        type="json",
        auth="public",
        website=True,
    ...
    def get_state_city(self, **kw):
        ...
        if state_id_:
            query = (
                "select code, id, name  from res_country_state_city where state_id = '"
                + str(state_id_)
                + "'"
            )
            request.cr.execute(query)
