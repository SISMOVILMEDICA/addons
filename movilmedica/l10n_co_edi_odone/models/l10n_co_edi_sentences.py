...
def render_xml_edi(values, template_name):
    base_path = path.dirname(path.dirname(__file__))
    env = Environment(loader=FileSystemLoader(path.join(base_path, "l10n_co_edi_xml")))
...
def get_cufe_cude(
    ...
    # CUFE = SHA-384(NumFac + FecFac + HorFac + ValFac + CodImp1 + ValImp1 + CodImp2 + ValImp2 + CodImp3 + ValImp3 + ValTot + NitFE + NumAdq + ClTec + TipoAmbie)
    # CUDE = SHA-384(NumFac + FecFac + HorFac + ValFac + CodImp1 + ValImp1 + CodImp2 + ValImp2 + CodImp3 + ValImp3 + ValTot + NitFE + NumAdq + Software-PIN + TipoAmbie)
    uncoded_value = (
        "%s + %s + %s + %s + %s + "
        "%s + %s + %s + %s + %s + "
        "%s + %s + %s + %s + %s"
    ...
    cufe_cude = hashlib.sha384(unicode_value.encode()).hexdigest()

    return {"CUFE/CUDEUncoded": uncoded_value, "CUFE/CUDE": cufe_cude}
...
def get_xml_with_signature(
    ...
    signature = xmlsig.template.create(
        xmlsig.constants.TransformInclC14N, xmlsig.constants.TransformRsaSha512, signature_id
    )
    ...
    props = template.create_signed_properties(qualifying, name=signature_id + "-signedprops")
    template.add_claimed_role(props, "supplier")
    policy = GenericPolicyId(
        signature_policy_url, signature_policy_description, xmlsig.constants.TransformSha512
    )
...
def get_pkcs12(certificate_file, certificate_password):
    try:
        ...
        raise ValidationError(
            _("The cretificate password or certificate file is not" " valid.\nException: %s") % e
        )


def get_qr_code(data):
    qr = QRCode(version=1, error_correction=constants.ERROR_CORRECT_L, box_size=20, border=4)
    qr.add_data(data)
    qr.make(fit=True)
    img = qr.make_image()
    # temp = StringIO()
...
def get_xml_soap_values(certificate_file, certificate_password):
    ...
    der = b64encode(crypto.dump_certificate(crypto.FILETYPE_ASN1, cert)).decode("utf-8", "ignore")

    return {"Created": Created, "Expires": Expires, "Id": uuid4(), "BinarySecurityToken": der}
