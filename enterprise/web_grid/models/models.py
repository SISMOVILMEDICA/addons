...
class Base(models.AbstractModel):
    ...
    def read_grid(
        ...
        grid_select = {col_field, cell_field}

        # readonly field should be in select clause with group_operator, or in group by clause too
        if readonly_field:
            grid_select.add(readonly_field)
            if (
                readonly_field != column_info.grouping
                and not self._fields[readonly_field].group_operator
            ):
                raise UserError(
                    _("The field used as readonly type must have a group_operator attribute.")
                )
    ...
    def _grid_get_row_headers(self, row_fields, groups, key):
        ...
        return [{"values": values, "domain": expression.OR(domains)} for values, domains in rows]
    ...
    def _grid_column_info(self, name, range):
        ...
        if field.type == "selection":
            ...
            return ColumnMetadata(
                ...
                initial=False,
                values=[
                    {"values": {name: v}, "domain": [(name, "=", v[0])], "is_current": False}
            ...
            return ColumnMetadata(
                ...
                values=[
                    {
                        "values": {name: self._get_date_column_label(d, field, span, step)},
                        "domain": [
                            "&",
            ...
            return ColumnMetadata(
                grouping="{}:{}".format(name, step),
                domain=["&", (name, ">=", r.start_utc), (name, "<=", r.end_utc)],
                prev=pagination.get("prev"),
                next=pagination.get("next"),
                initial=pagination.get("initial"),
                values=[
                    {
                        "values": {name: self._get_date_column_label(d[0], field, span, step)},
                        "domain": [
                            "&",
    ...
    def _get_date_column_label(self, date, field, span, step):
        ...
        if (
            field.type == "datetime"
        ):  # we want the column label to be the infos in user tz, while the date domain should still be in UTC
            _date_tz = date.astimezone(pytz.timezone(self._context.get("tz") or "UTC"))
        else:
            _date_tz = date

        return (
            "{}/{}".format(field.to_string(date), field.to_string(date + self._grid_step_by(step))),
            _labelize(_date_tz),
        )
    ...
    def _get_date_formatter(self, step, field, locale):
        ...
        if step == "week":
            ...
            def _week_format(date):
                ...
                return _("Week %(weeknumber)s\n%(week_start)s - %(week_end)s") % {
                    ...
                    "week_end": format_date(
                        self.env,
                        date + self._grid_step_by(step) - relativedelta(days=1),
                        locale,
                        "MMM\u00A0dd",
                    ),
                }
        ...
        if hasattr(babel.dates, "format_skeleton"):
            ...
            return partial(babel.dates.format_date, format=FORMAT[step], locale=locale)
    ...
    def _grid_pagination(self, field, span, step, anchor):
        if field.type in ["date", "datetime"]:
            ...
            return {
                ...
                "initial": {
                    "grid_anchor": field.to_string(today),
                    "default_%s" % field.name: field.to_string(today),
                },
            }
    ...
    def _grid_range_of(self, span, step, anchor, field):
        ...
        if field.type == "datetime":
            ...
            return datetime_range(
                self._grid_start_of(span, step, anchor),
                self._grid_end_of(span, step, anchor),
                step,
                user_tz,
            )
        return date_range(
            self._grid_start_of(span, step, anchor), self._grid_end_of(span, step, anchor)
        )
    ...
    def _grid_date_is_current(self, field, span, step, date):
        ...
        if step == "day":
            ...
            return (
                self._grid_start_of_period(span, step, date)
                <= today
                < self._grid_end_of_period(span, step, date)
            )
        return False
...
class datetime_range(object):
    ...
    def _generate_period(self):
        ...
        stop = (
            self._end.replace(tzinfo=None) + STEP_BY[self.step]
        )  # add a step as the given stop limit is not included in column domain
        ...
        for dt in r:
            ...
            if previous_dt:
                date_range.append(
                    (previous_dt.astimezone(pytz.utc), current_dt.astimezone(pytz.utc))
                )
