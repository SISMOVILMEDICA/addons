...
class AccountBankStatementImport(models.TransientModel):
    ...
    def import_file(self):
        ...
        for data_file in self.attachment_ids:
            ...
            if not journal:
                ...
                return self.with_context(active_id=self.ids[0])._journal_creation_wizard(
                    currency, account_number
                )
            if not journal.default_account_id:
                raise UserError(
                    _("You have to set a Default Account for the journal: %s") % (journal.name,)
                )
    ...
    def _find_additional_data(self, currency_code, account_number):
        ...
        if journal:
            ...
            if currency and currency != journal_currency:
                ...
                journal_cur_code = (
                    not journal_currency and company_currency.name or journal_currency.name
                )
                raise UserError(
                    _(
    ...
    def _create_bank_statements(self, stmts_vals):
        ...
        for st_vals in stmts_vals:
            filtered_st_lines = []
            for line_vals in st_vals["transactions"]:
                if line_vals["amount"] != 0 and (
                    "unique_import_id" not in line_vals
                    or not line_vals["unique_import_id"]
                    or not bool(
                        BankStatementLine.sudo().search(
                            [("unique_import_id", "=", line_vals["unique_import_id"])], limit=1
                        )
                    )
                ):
            ...
            notifications += [
                {
                    "type": "warning",
                    "message": _(
                        "%d transactions had already been imported and were ignored.", num_ignored
                    )
                    if num_ignored > 1
                    ...
                    "details": {
                        ...
                        "ids": BankStatementLine.search(
                            [("unique_import_id", "in", ignored_statement_lines_import_ids)]
                        ).ids,
                    },
                }
            ]
